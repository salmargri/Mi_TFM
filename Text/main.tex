...
Now we want to generalize the basic ideas present in the Shor code to give a simple picture of how we can detect and correct errors in a quantum system. We firstly define a quantum error-correcting code as a subspace $C$ of the total Hilbert space. We encode our quantum states by projecting them into $C$ by an unitary operation $P$ that we will call the projector. The enconded system is then vulnerable to enviromental noise, which we model as a CPTP map $\mathcal{E}$. We perform a syndrome measurement to determine the error that has affected the system, and a recovery operation $\mathcal{R}$ returns the system to its original encoded state. \\
